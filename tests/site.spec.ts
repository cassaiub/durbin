...
test("exhibition popup supports images, object navigation, and focus restoration", async ({ page }) => {
  ...
  expect((boundedImage?.x ?? 0) + (boundedImage?.width ?? 0)).toBeGreaterThanOrEqual((fullscreenViewport?.x ?? 0) + (fullscreenViewport?.width ?? 0) - 1);
  expect((boundedImage?.y ?? 0) + (boundedImage?.height ?? 0)).toBeGreaterThanOrEqual((fullscreenViewport?.y ?? 0) + (fullscreenViewport?.height ?? 0) - 1);
...
test("the About page begins with the programme story and clear next steps", async ({ page }) => {
  ...
  await expect(scrollVideo.locator("source")).toHaveAttribute("src", "/videos/pixabay-space-universe-204735.mp4");
  ...
  await expect.poll(() => horizontal.evaluate((node) => Number.parseFloat(getComputedStyle(node).getPropertyValue("--about-progress")))).toBeGreaterThan(.35);
